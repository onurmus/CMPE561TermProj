...
<body>
	...
	function doStuff($count,$conn,$whereWeLeft){
		...
		$sql1 = "SELECT distinct(userId) FROM `tbltweets` WHERE hashtag in ('drunk','drank','imdrunk') LIMIT 3900 OFFSET 0";
		...
		if ($result->num_rows > 0) {
		    ...
		    while($row = $result->fetch_assoc()) {
				...
				for ($k=0; $k < $size; $k++) { 
					...
					$sql = "INSERT INTO `dataset2`(`userId`, `tweetId`, `hashtag`, `tweet`)  
	...
	while ($whereWeLeft < 3900) {
		try{
			$count  = 0;
			doStuff($count,$conn,$whereWeLeft);
		}catch(Exception $e){
			$whereWeLeft = $count;
			echo "whereWeLeft :". $whereWeLeft. "<br>";
		}
	}
