	...
	<?php
		...
		if ($result->num_rows > 0) {
		    ...
		    while($row = $result->fetch_assoc()) {
		        ...
		        $getfield = '?lang=en&count=200&user_id='.number_format($currentUserId,0,'','');//.'&max_id='.number_format($maxID,0,'','');
				...
				for ($k=0; $k < $size; $k++) { 
					...
					$currentTweetText = $conn->real_escape_string($currentTweet->text);
					//if tweet contains sober tags, then save
					if(strpos($currentTweetText, 'notdrunk') !== false || strpos($currentTweetText, 'imnotdrunk') !== false 
							|| strpos($currentTweetText, 'sober') !== false){
						echo "burada <br>";

						//and if tweet does not contain drunk hashtags
						if(strpos($currentTweetText, 'drank') !== true && strpos($currentTweetText, 'drunk') !== true 
							&& strpos($currentTweetText, 'imdrunk') !== true){

							$sql = "INSERT INTO `dataset3v2`(`userId`, `tweetId`, `hashtag`, `tweet`)  
							VALUES ('".$currentUserId."', '".$currentTweet->id_str."', ' ','".$currentTweetText."')";
